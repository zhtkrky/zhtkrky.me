      ...
      <div className="mb-4 flex flex-col justify-between gap-3">
        {blogs?.map((blog) => (
          <Link
            href={`/${blog.slug}`}
            key={blog.id}
            className="-mx-4 flex flex-col items-start rounded-md px-4 no-underline dark:hover:bg-neutral-900 hover:bg-neutral-100 py-4"
          >
            <div className="flex w-full justify-between items-center mb-2">
              <h3>{blog.title}</h3>
              <span className="text-sm hidden md:block text-neutral-400 dark:text-neutral-500">
